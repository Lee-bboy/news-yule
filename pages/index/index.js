...
const appKey = '65df79c776bf5658a43613ba3907ef6a' // 用于访问新闻接口的appKey
...
Page({
  ...
  viewDetail: function(e) {
    ...
    let arr = newsUrl.split('=');
    // console.log('arr:' +arr);
    let n_url =  decodeURIComponent(arr[1]);
    console.log('n_url1:' +n_url);

    wx.navigateTo({
      url: '../detail/detail?n_url=' + n_url
    })
  },
  ...
  renderPage: function(newsType, isRefresh, calllBack) {
    if (!isRefresh) {
      ...
      request({ url: `https://v.juhe.cn/toutiao/index?type=yule&key=${appKey}`, newstype: newsType })
        .then(res => {
          wx.hideLoading()
